...
type Moment = { id: Target; caption: string; placement: 'after-body' | 'after-pair' | 'before-final' };

const STORY_URL = 'https://www.eastokyo.com/he-never-really-left-the-arena';
const TYPES: ReactionType[] = ['like', 'love', 'wow'];
const MOMENTS: Moment[] = [
  { id: 'arena-article', caption: 'IL EST TOUJOURS RETOURNÉ DANS L’ARÈNE — EASTOKYO', placement: 'after-body' },
  { id: 'arena-drama', caption: 'L’ARÈNE, C’EST DU DRAME — EASTOKYO', placement: 'after-pair' },
  { id: 'arena-return', caption: 'ENCORE ET ENCORE — PICASSO ET L’ARÈNE — EASTOKYO', placement: 'before-final' },
];
...
function links(text: string) {
  const u = encodeURIComponent(STORY_URL);
  const t = encodeURIComponent(text);
  return {
    pinterest: `https://www.pinterest.com/pin/create/button/?url=${u}&description=${t}`,
    facebook: `https://www.facebook.com/sharer/sharer.php?u=${u}`,
    x: `https://twitter.com/intent/tweet?text=${t}&url=${u}`,
    bluesky: `https://bsky.app/intent/compose?text=${encodeURIComponent(`${text} ${STORY_URL}`)}`,
  };
}
...
export default function ArenaEngagement() {
  const [mounts, setMounts] = useState<Record<Target, HTMLElement> | null>(null);
  const [states, setStates] = useState<Record<Target, ReactionState>>({ 'arena-article': emptyState(), 'arena-drama': emptyState(), 'arena-return': emptyState() });
  ...
  useEffect(() => {
    ...
    const main = document.querySelector('.arena-story-theme main#top');
    if (!main) return;
    const created = {} as Record<Target, HTMLElement>;

    MOMENTS.forEach((moment) => {
      let anchor: Element | null = null;
      let before = false;
      if (moment.placement === 'after-body') anchor = main.querySelector('section[class*="body"]');
      if (moment.placement === 'after-pair') anchor = main.querySelector('div[class*="bullfightPair"]');
      if (moment.placement === 'before-final') { anchor = main.querySelector('section[class*="finalAct"]'); before = true; }
      if (!anchor?.parentElement) return;
      const mount = document.createElement('div');
      mount.className = shareStyles.momentMount;
      if (before) anchor.parentElement.insertBefore(mount, anchor);
      else anchor.parentElement.insertBefore(mount, anchor.nextSibling);
  ...
  useEffect(() => {
    ...
    })).then((rows) => setStates((current) => Object.fromEntries([...Object.entries(current), ...rows.filter(Boolean) as Array<readonly [Target, ReactionState]>]) as Record<Target, ReactionState>)).catch(() => {});
  }, [visitorId]);

  const nativeShare = async (text: string) => {
    try {
      if (navigator.share) await navigator.share({ title: 'EASTOKYO', text, url: STORY_URL });
      else { await navigator.clipboard.writeText(STORY_URL); setStatus('LIEN COPIÉ'); window.setTimeout(() => setStatus(''), 1600); }
    } catch (error) { if ((error as DOMException)?.name !== 'AbortError') setStatus('PARTAGE NON TERMINÉ'); }
  };

  const copy = async () => {
    try { await navigator.clipboard.writeText(STORY_URL); setStatus('LIEN COPIÉ'); window.setTimeout(() => setStatus(''), 1600); }
  ...
  const Rail = ({ moment }: { moment: Moment }) => {
    const direct = links(moment.caption);
    const targetBusy = busy.has(moment.id);
    return <div className={shareStyles.rail} aria-label="Partager cet article">
      <button className={`${shareStyles.cell} ${shareStyles.lead}`} type="button" onClick={() => nativeShare(moment.caption)} aria-label="Partager l’article"><Icon name="share"/><span className={shareStyles.leadText}>PARTAGER L’ARTICLE</span></button>
      <button className={shareStyles.cell} type="button" onClick={() => nativeShare(moment.caption)} aria-label="Partager vers Instagram"><Icon name="instagram"/></button>
      <button className={shareStyles.cell} type="button" onClick={() => nativeShare(moment.caption)} aria-label="Partager vers TikTok"><Icon name="tiktok"/></button>
      <a className={shareStyles.cell} href={direct.pinterest} target="_blank" rel="noreferrer" aria-label="Partager sur Pinterest"><Icon name="pinterest"/></a>
      <a className={shareStyles.cell} href={direct.facebook} target="_blank" rel="noreferrer" aria-label="Partager sur Facebook"><Icon name="facebook"/></a>
      <a className={shareStyles.cell} href={direct.x} target="_blank" rel="noreferrer" aria-label="Partager sur X"><Icon name="x"/></a>
      <a className={shareStyles.cell} href={direct.bluesky} target="_blank" rel="noreferrer" aria-label="Partager sur Bluesky"><Icon name="bluesky"/></a>
      <button className={`${shareStyles.cell} ${shareStyles.copy}`} type="button" onClick={copy} aria-label="Copier le lien"><Icon name="copy"/></button>
      <span className={reactionStyles.host}><span className={reactionStyles.cluster} aria-label="Réactions — choisissez une seule réaction">
        {TYPES.map((type) => {
          const value = states[moment.id][type];
          const label = type === 'like' ? 'Like' : type === 'love' ? 'Love' : 'Wow';
          return <button key={type} type="button" className={`${reactionStyles.reaction} ${reactionStyles[type]} ${value.active ? reactionStyles.active : ''} ${targetBusy ? reactionStyles.loading : ''}`} onClick={() => toggle(moment.id, type)} disabled={targetBusy} aria-pressed={value.active} aria-label={`${label} — ${value.count}`}>
  ...
  return <>{mounts ? MOMENTS.map((moment) => mounts[moment.id] ? createPortal(<Rail moment={moment}/>, mounts[moment.id], moment.id) : null) : null}{status && <div className={shareStyles.toast} role="status">{status}</div>}</>;
}
