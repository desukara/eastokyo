...
const MOMENTS: Moment[] = [
  { id: 'index-object', article: 1, text: 'A BICYCLE BECOMES A BULL — EASTOKYO' },
  { id: 'index-rayures', article: 3, text: 'PAUL SMITH ADDS HIS TOUCH — EASTOKYO' },
...
export default function IndexEngagement(){
  ...
  useEffect(()=>{
    setVid(visitor());
    const entries=document.querySelector('main#top section[aria-label="Picasso Index entries"]');
    if(!entries) return;
  ...
  const Rail=({m}:{m:Moment})=>{const l=links(m.text);return <div className={shareStyles.rail} aria-label="Share this section">
    <button className={`${shareStyles.cell} ${shareStyles.lead}`} onClick={()=>share(m.text)} aria-label="Share this section"><Icon name="share"/><span className={shareStyles.leadText}>SHARE THIS SECTION</span></button>
    <button className={shareStyles.cell} onClick={()=>share(m.text)} aria-label="Share to Instagram using your device"><Icon name="instagram"/></button><button className={shareStyles.cell} onClick={()=>share(m.text)} aria-label="Share to TikTok using your device"><Icon name="tiktok"/></button>
    <a className={shareStyles.cell} href={l.p} target="_blank" rel="noreferrer" aria-label="Share on Pinterest"><Icon name="pinterest"/></a><a className={shareStyles.cell} href={l.f} target="_blank" rel="noreferrer" aria-label="Share on Facebook"><Icon name="facebook"/></a><a className={shareStyles.cell} href={l.x} target="_blank" rel="noreferrer" aria-label="Share on X"><Icon name="x"/></a><a className={shareStyles.cell} href={l.b} target="_blank" rel="noreferrer" aria-label="Share on Bluesky"><Icon name="bluesky"/></a><button className={`${shareStyles.cell} ${shareStyles.copy}`} onClick={()=>navigator.clipboard.writeText(URL)} aria-label="Copy EASTOKYO link"><Icon name="copy"/></button>
    <span className={reactionStyles.host}><span className={reactionStyles.cluster} aria-label="Reactions — choose one reaction">{TYPES.map(type=>{const v=states[m.id][type];const label=type==='like'?'Like':type==='love'?'Love':'Wow';return <button key={type} className={`${reactionStyles.reaction} ${reactionStyles[type]} ${v.active?reactionStyles.active:''}`} disabled={busy.has(m.id)} onClick={()=>toggle(m.id,type)} aria-pressed={v.active} aria-label={`${label} — ${v.count}`}><span className={reactionStyles.icon}><RIcon type={type}/></span><span className={reactionStyles.count}>{v.count}</span></button>;})}</span></span>
  </div>};
