  ...
  return <main id="top" className={styles.page}>
    ...
    <section className={styles.bodyBeat}>
      ...
      <div className={`${styles.prose} ${styles.openingB}`}>
        <p>C'est tout le propos de l'exposition : vous remettre devant quelque chose que vous pensiez déjà connaître, et vous demander de regarder une deuxième fois.</p>
        ...
        <p>Mais pourquoi il n'y aurait qu'un seul angle valable sur quoi que ce soit ? Peut-être parce qu'une réponse fixe rassure, dans un monde qui refuse obstinément de rester en place. C'est possible. C'est aussi, franchement, un peu chiant, et je préfère garder les yeux ouverts pour la version que je n'ai pas encore vue.</p>
      </div>
    ...
    <section className={styles.blueAct}><div className={styles.blueInner}><div className={styles.blueHead}><p className={styles.sectionKicker}>03 · RYTHME</p><h2>Et puis la salle commence à prendre vie.</h2></div><figure className={styles.blueFigure}><picture><source media="(max-width: 760px)" srcSet="/images/editorial/picasso-striped-gallery-mobile.png"/><img src="/images/editorial/picasso-striped-gallery-desktop.png" alt="Galerie de l’exposition Picasso avec un traitement mural rayé" loading="lazy"/></picture><Caption>AJOUTER UNE NOUVELLE COUCHE N'EFFACE PAS LA PREMIÈRE. ELLE LA REND PLUS JUSTE.</Caption></figure><div className={styles.blueText}>
      <p>Dans l'une des galeries, Smith couvre le mur de rayures du sol au plafond, une de ses signatures depuis des décennies, à des années-lumière du geste de Picasso. Picasso a créé l'œuvre, Smith lui donne un nouvel endroit où exister. Ça dépasse l'éclairage soigné ou le joli fond : ça ressemble presque à un partenaire de scène. À partir de là, l'exposition ne ressemble déjà plus tout à fait à celle dans laquelle on était entré. Deux formes de génie très différentes se retrouvent dans la même pièce, chacune regardant le monde à sa manière.</p>
    ...
    <section className={styles.quiet}>
      <div className={styles.quietHead}><p className={styles.sectionKicker}>04 · FAITES ATTENTION</p><h2>Le plus beau, c'est de voir quelqu'un d'autre s'arrêter net.</h2><div className={styles.prose}>
        ...
        <p>Des mondes entiers se plient à l'intérieur d'autres mondes, ici. Une exposition, ou une ville d'ailleurs, peut contenir plusieurs millions de perceptions privées en même temps. À ce compte-là, le mot « art » paraît presque trop petit pour ce qui se passe réellement. Le cadre sert peut-être à ça, finalement : ouvrir une porte vers le sens plutôt que d'y bâtir un mur.</p>
      </div></div>
      <div className={styles.quietFinal}><div className={`${styles.prose} ${styles.finalMain}`}>
        <p>Le don de Picasso tenait à autre chose qu'un accès réservé à un territoire dont on aurait tous été exclus. Plus simple, et plus difficile en même temps. Il continuait simplement à regarder longtemps après que les autres avaient décidé ce que l'image était censée vouloir dire. Réussir ça une fois est déjà difficile. Continuer pendant soixante ans relève d'une obstination assez rare.</p>
      ...
      <figure className={styles.thresholdFigure}><picture><source media="(max-width: 760px)" srcSet="/images/editorial/picasso-threshold-gallery-mobile.png"/><img src="/images/editorial/picasso-threshold-gallery-desktop.png" alt="Une personne traverse la galerie de l’exposition Picasso" loading="lazy"/></picture><Caption>LE CADRE S'ARRÊTE. LE REGARD, NON.</Caption></figure>
