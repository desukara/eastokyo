...
export default function ExhibitionStoryPage() {
  return <main id="top" className={styles.page}>
    ...
    <section className={styles.intro}><div className={styles.introLabel}>01 · LOOK AGAIN</div><p className={styles.standfirst}>An exhibition is so much more than just the art on the wall. There's everything in the room around them too, and above all else the imagination we bring with us.</p><aside className={styles.introNote}>Paul Smith doesn't try to hide in the room behind Pablo Picasso. He transforms the whole space around him entirely, and that's precisely where the art pieces start speaking to us.</aside></section>
    ...
    <section className={styles.bodyBeat}>
      ...
      <div className={`${styles.prose} ${styles.openingA}`}>
        <p>There's an elegant woman walking toward the museum under a blue parasol, in the sweltering heat of August, arriving as someone who figured out long ago how to survive all this heat. I look up: not a single grey cloud. Summer in Tokyo is a lot like an antechamber of hell, with burning asphalt throwing the heat right back at your head, and for a split second that open parasol strikes me as completely bizarre.</p>
        <p>Then I remember where I am geographically. Here, in Japan, an umbrella doesn't necessarily mean what I always thought it did.</p>
      ...
      <div className={`${styles.prose} ${styles.openingB}`}>
        ...
        <p>Picasso is a perfect candidate for that, because there's almost too much Picasso to hold in your head at once. Cubism, Guernica, the Blue Period, the Rose Period, auction prices that almost nobody can afford, faces where the nose seems to answer to a completely different department from the eyes. Picasso is one of those names that stopped referring to a person and has become a kind of general-knowledge fact, something absorbed by osmosis without ever setting one foot in a museum. Everyone knows Picasso, at least a little.</p>
        <p>And that's where it gets complicated. Knowing something so well that you no longer see it as a strange trap, and not one confined to art. It happens all the time in relationships too. At first, every detail about the other person is an amazing new discovery. Then familiarity arrives, and that familiarity curdles into "I already know how this sentence ends." We finish the other person's thoughts and call it intimacy, when half the time it actually means we stopped listening. Recognising things isn't the problem. It's essential, nobody can spend their life re-examining everything every five minutes. But certainty is something else. It folds its arms and thinks it already knows everything about everything.</p>
        <p>An umbrella is for rain. Picasso is Cubism. It’s already decided.</p>
        <p>But who decided there should be only one valid angle on anything at all? Maybe because an authoritative answer is reassuring in a world that can be terrifying at times. OK, possibly. Such conclusions are also, quite frankly, fucking boring, and I'd rather keep my eyes open for the versions I haven't seen yet.</p>
      </div>
    ...
    <section className={styles.harlequin}>
      ...
      <div className={styles.harlequinLead}><p className={styles.sectionKicker}>02 · COLOUR ENTERS THE CONVO</p><h2>Then the wall stops playing the innocent role.</h2><div className={styles.prose}>
        <p>Paul Smith isn't really trying to reinterpret Picasso. He builds him a stage instead, and lets that stage join the show. Colour arrives as the leading lady. Stripes climb the walls like villains, patterns press against other patterns, and nothing stays pleasantly in its place the way a museum trains you to expect. Take the star of the performance, Harlequin: a radiant figure already made of diamonds and bold flat areas, hanging in front of a wall that definitely has something to say about it. The costume and the wallpaper get into a cat fight, and suddenly the lines and colours we assumed we knew yell at each other from one end of the room to the other, pissed about the perfect little frames museums have taught us to believe in.</p>
        <p>Smith doesn't repaint anything. Why should he? He arranges things so the room and the painting draw out of each other something neither could have produced alone. Without anything actually moving, the painting starts living again in your mind. Obviously, the canvas isn't going to crawl through the gallery like some cheap sequel to <em>Night at the Museum</em> filmed in Málaga. It stays perfectly still. It's your soul that moved, and voilà… an image that five minutes earlier looked like an old acquaintance pulled from an art-history textbook gets its life back.</p>
        <p>What Smith brings out was already true, just easy to forget; yet meaning has never stayed politely locked inside its little frame. Nothing in the exhibition is just a background, once you dare to look at it a little sideways. From the paintings on the wall to the walls of the room. Then from the room inside the museum to the museum in the middle of Tokyo. More than that, Tokyo is in the middle of Japan and all of it is on Planet Earth. All things are interconnected and nothing is isolated from the rest of the world. After a while you start wondering where anything in life actually stops.</p>
    ...
    <section className={styles.blueAct}><div className={styles.blueInner}><div className={styles.blueHead}><p className={styles.sectionKicker}>03 · RHYTHM</p><h2>And then the room speaks for itself.</h2></div><figure className={styles.blueFigure}><picture><source media="(max-width: 760px)" srcSet="/images/editorial/picasso-striped-gallery-mobile.png"/><img src="/images/editorial/picasso-striped-gallery-desktop.png" alt="Picasso exhibition gallery with striped wall treatment" loading="lazy"/></picture><Caption>ADDING A NEW LAYER DOES NOT ERASE THE FIRST. IT MAKES IT MORE ACCURATE.</Caption></figure><div className={styles.blueText}>
      <p>In one gallery, Smith covers the wall in stripes from floor to ceiling, his artistic signature for decades, light-years from Picasso's gesture. Yes, Picasso made the work. Smith gives it a new place to let loose. It goes far beyond careful lighting or a magnificent backdrop. It’s a stage co-star. From this point the exhibition in no way resembles the one you walked into. Two very different forms of remarkable genius become roommates, each viewing the world uniquely.</p>
      <p>So we see now that no single way of seeing a painting gets to be the one true view. Because different views strengthen each other. Art stays fluid like a flowing conversation: new people arrive with their own way of seeing it with their own eyes, and the masterpiece doesn't weaken. Because someone is reviving the work every time it finds another life.</p>
    </div></div></section>
    ...
    <section className={styles.quiet}>
      <div className={styles.quietHead}><p className={styles.sectionKicker}>04 · PAY ATTENTION &amp; SEE</p><h2>The most beautiful thing is watching someone else truly experience the art.</h2><div className={styles.prose}>
        ...
        <p>Picasso's miraculous gift of imagination and brilliant ways of expressing himself didn't come from privileged access to some restricted area. On the contrary. He just kept looking long after everyone else told him what to think. Managing to do that can be difficult. Keeping it up for sixty years takes a rare kind of brilliance.</p>
        <p>The object in front of you doesn't need to change for you to truly see it. What must change is how much you're actually willing to notice if you wake up and make the effort. Once that really happens, the city outside the museum is way more beautiful and exciting. Tokyo simply continues, the way things are always changing meaning depending on who's cherishing them, and whatever happens to be beside them that day.</p>
        <p>Life keeps spilling over and rebuilding everywhere, whether we regard it or not. So here's the challenge: find one thing you're absolutely certain you've already figured out, and start again with that exact one thing.</p>
      </div></div>
