...
type Moment = { id: Target; section: number; caption: string };

const STORY_URL = 'https://www.eastokyo.com/picasso-seen-with-fresh-eyes';
const TYPES: ReactionType[] = ['like', 'love', 'wow'];
const MOMENTS: Moment[] = [
  { id: 'picasso-opening', section: 3, caption: 'PICASSO, SOUS UN AUTRE REGARD — EASTOKYO' },
  { id: 'picasso-colour', section: 4, caption: 'LA COULEUR ENTRE DANS LA DISCUSSION — EASTOKYO' },
  { id: 'picasso-rhythm', section: 5, caption: 'ET PUIS LA SALLE COMMENCE À PRENDRE VIE — EASTOKYO' },
  { id: 'picasso-finale', section: 7, caption: 'PICASSO VU PAR PAUL SMITH — EASTOKYO' },
];
...
function optimisticState(before: ReactionState, reaction: ReactionType, action: 'add' | 'remove'): ReactionState {
  return Object.fromEntries(TYPES.map((type) => {
    ...
    if (action === 'remove') {
      return [type, type === reaction ? { count: Math.max(0, value.count - 1), active: false } : value];
    }
...
export default function PicassoEngagement() {
  const [mounts, setMounts] = useState<Record<Target, HTMLElement> | null>(null);
  const [states, setStates] = useState<Record<Target, ReactionState>>({ 'picasso-opening': emptyState(), 'picasso-colour': emptyState(), 'picasso-rhythm': emptyState(), 'picasso-finale': emptyState() });
  ...
  useEffect(() => {
    ...
    MOMENTS.forEach((moment) => {
      const anchor = sections[moment.section - 1];
  ...
  useEffect(() => {
    ...
    Promise.all(MOMENTS.map(async ({ id }) => {
      const response = await fetch(`/api/picasso-reactions?target=${id}&visitorId=${encodeURIComponent(visitorId)}`, { cache: 'no-store' });
  ...
  const nativeShare = async (text: string) => {
    try {
      ...
      else await navigator.clipboard.writeText(STORY_URL);
    } catch (error) { if ((error as DOMException)?.name !== 'AbortError') console.error(error); }
  };

  const copy = async () => { try { await navigator.clipboard.writeText(STORY_URL); } catch {} };
  ...
  const toggle = async (target: Target, reaction: ReactionType) => {
    ...
    try {
      const response = await fetch('/api/picasso-reactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ target, reaction, visitorId, action }) });
  ...
  const Rail = ({ moment }: { moment: Moment }) => {
    ...
    return <div className={shareStyles.rail} aria-label="Partager cet article">
      <button className={`${shareStyles.cell} ${shareStyles.lead}`} type="button" onClick={() => nativeShare(moment.caption)}><Icon name="share"/><span className={shareStyles.leadText}>PARTAGER L’ARTICLE</span></button>
      ...
      <span className={reactionStyles.host}><span className={reactionStyles.cluster} aria-label="Réactions — choisissez une seule réaction">
        {TYPES.map((type) => {
          ...
          return <button key={type} type="button" className={`${reactionStyles.reaction} ${reactionStyles[type]} ${value.active ? reactionStyles.active : ''}`} disabled={targetBusy} onClick={() => toggle(moment.id, type)} aria-pressed={value.active}><span className={reactionStyles.icon}><ReactionIcon type={type}/></span><span className={reactionStyles.count}>{value.count}</span></button>;
        })}
      </span></span>
    </div>;
  };

  return <>{MOMENTS.map((moment) => mounts?.[moment.id] ? createPortal(<Rail moment={moment}/>, mounts[moment.id], moment.id) : null)}</>;
}
